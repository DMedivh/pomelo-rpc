...
pro.stop = function (force) {
  if (this.state !== STATE_STARTED) {
    logger.warn('client is not running now.');
    return;
  }
...
pro.dispatch = function (tracer, serverId, msg, opts, cb) {
  ...
  if (this.state !== STATE_STARTED) {
    ...
    logger.error('client is not running now.');
  ...
  var send = function (tracer, err, serverId, msg, opts) {
    ...
    if (!mailbox) {
      ...
      logger.error('could not find mailbox with id:', { serverId });
    ...
    mailbox.send(tracer, msg, opts, function (tracer_send, send_err, args) {
      ...
      if (send_err) {
        logger.error('fail to send message', { message: send_err.stack || send_err.message });
...
pro.connect = function (tracer, serverId, cb) {
  ...
  mailbox.connect(tracer, function (err) {
    if (!!err) {
      ...
      logger.error('mailbox fail to connect to remote server: ', { serverId });
...
var lazyConnect = function (tracer, station, serverId, factory, cb) {
  ...
  if (!server) {
    logger.error('unknown server', { serverId });
    return false;
  }
  if (!online || online !== 1) {
    logger.error('server is not online', { serverId });
    return false;
  }
...
var addToPending = function (tracer, station, serverId, args) {
  ...
  if (pending.length > station.pendingSize) {
    ...
    logger.warn('station pending too much for', { serverId });
    return;
  }
  pending.push(args);
};
...
var flushPending = function (tracer, station, serverId, cb) {
  ...
  if (!mailbox) {
    ...
    logger.error('fail to flush pending messages for empty mailbox ', { serverId });
  }
...
var errorHandler = function (tracer, station, err, serverId, msg, opts, flag, cb) {
  if (!!station.handleError) {
    ...
    logger.error('rpc filter error', { serverId, message: err.stack || err.message });
