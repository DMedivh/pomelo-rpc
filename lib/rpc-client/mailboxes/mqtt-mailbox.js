...
MailBox.prototype.connect = function (tracer, cb) {
  ...
  this.socket.on('publish', function (pkg) {
    ...
    try {
      ...
      logger.error('rpc client  with error', { serverId: self.serverId, id: self.id, message: err.stack });
    }
  });

  this.socket.on('error', function (err) {
    logger.error('rpc socket is error', { serverId: self.serverId, id: self.id, remotehost: self.host, remoteport: self.port });
    self.emit('close', self.id);
  });
...
var clearCbTimeout = function (mailbox, id) {
  if (!mailbox.timeout[id]) {
    logger.warn('timer is not exsits', { serverId: mailbox.serverId, id, host: mailbox.host, port: mailbox.port });
    return;
  }
