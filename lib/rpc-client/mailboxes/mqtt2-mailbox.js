...
MailBox.prototype.connect = function (tracer, cb) {
  ...
  var connectTimeout = setTimeout(function () {
    logger.error('rpc client timeout', { serverId: self.serverId, id: self.id });
  ...
  this.socket.on('publish', function (pkg) {
    ...
    try {
      ...
      logger.error('rpc client with error', { serverId: self.serverId, id: self.id, message: err.message || err.stack });
    }
  });

  this.socket.on('error', function (err) {
    logger.error('rpc socket is error', { serverId: self.serverId, id: self.id, remote: self.host, port: self.port });
    self.emit('close', self.id);
    self.close();
  });
  ...
  this.socket.on('disconnect', function (reason) {
    logger.error('rpc socket is disconnect', { serverId: self.serverId, id: self.id, reason });
  ...
  if (this.lastPing > 0) {
    if (this.lastPong < this.lastPing) {
      if (now - this.lastPing > KEEP_ALIVE_TIMEOUT) {
        logger.error('mqtt rpc client checkKeepAlive timeout', { serverId: this.serverId, id: this.id, KEEP_ALIVE_TIMEOUT, lastPing: this.lastPing, lastPong: this.lastPong });
...
var setCbTimeout = function (mailbox, id, tracer, cb) {
  ...
  var timer = setTimeout(function () {
    ...
    var eMsg = util.format('rpc callback timeout', { serverId: mailbox.serverId, timeoutValue: mailbox.timeoutValue, id, remote: mailbox.host, port: mailbox.port });
    logger.error(eMsg);
...
var clearCbTimeout = function (mailbox, id) {
  ...
  if (!mailbox.timeout[id]) {
    logger.warn('timer is not exsits', { serverId: mailbox.serverId, id, remote: mailbox.host, port: mailbox.port });
    return;
  }
