...
var proxyCB = function (client, serviceName, methodName, args, attach, isToSpecifiedServer) {
  if (client.state !== STATE_STARTED) {
    logger.error('fail to invoke rpc proxy for client is not running');
    return;
  }
  if (args.length < 2) {
    logger.error('invalid rpc invoke, arguments length less than 2',
      { namespace: attach.namespace, serverType: attach.serverType, serviceName, methodName });
    return;
  }
...
var rpcToSpecifiedServer = function (client, msg, serverType, serverId, cb) {
  if (typeof serverId !== 'string') {
    logger.error('serverId is not a string', { serverId });
    return;
  }
  if (serverId === '*') {
    var servers = client._routeContext.getServersByType(serverType);
    if (!servers) {
      logger.error('serverType servers not exist', { serverType });
      return;
    }
