var logger = require('pomelo-logger').getLogger('pomelo-rpc', 'rpc-proxy');
...
exp.create = function (opts) {
  if (!opts || !opts.origin) {
    logger.warn('proxy', { tips: 'opts and opts.origin should not be empty.' });
    return null;
  }

  if (!opts.proxyCB || typeof opts.proxyCB !== 'function') {
    logger.warn('proxy', { tips: 'opts.proxyCB is not a function, return the origin module directly.' });
    return opts.origin;
  }
